...
export default class Broadcasters extends Vue {
  channels: Channel[] = [];
  broadcasters: Broadcaster[] = [];
  loading: Boolean = true;

  async created() {
    const chnls: Channel[] = (await Axios.get(AppCongig.apiUrl + "/channel", {})).data;
    chnls.sort((c1, c2) => c2.subscriberCount - c1.subscriberCount);
    chnls.forEach(d => {
      this.channels.push(d);
    });

    const bros: Broadcaster[] = (await Axios.get(AppCongig.apiUrl + "/broadcaster", {})).data;
    bros.forEach(d => {
